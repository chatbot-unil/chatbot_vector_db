...
def process_demography(file_path):
    documents = []

    with open(file_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
        context = data.get('context', '')
        for year, stats in data.get('data', {}).items():
            document_content = json.dumps({
                "year": year,
                "stats": stats
            }, ensure_ascii=False)
...
if __name__ == "__main__":
    ...
    demography_data = "data/demographic_data/demographic_data_2011_2022.json"
    ...
    process_demography(demography_data)